...
import { bookingApi, type CancelBookingPayload } from '@/lib/booking';
...
export default function BookingsPage() {
  ...
  const loadBookings = useCallback(async () => {
    ...
    try {
      ...
      const result = await bookingApi.list(params as { page: number; limit: number; status?: string });
  ...
  const handleConfirm = async (bookingId: string) => {
    ...
    try {
      const updated = await bookingApi.confirm(bookingId);
      setBookings((prev) => prev.map((b) => (b.id === updated.id ? updated : b)));
      toast.success('Booking confirmed');
    } catch (err: unknown) {
      const error = err as { response?: { data?: { error?: { message?: string } } } };
      toast.error(error?.response?.data?.error?.message || 'Failed to confirm');
    } finally {
      setConfirming(null);
    }
  };
  ...
  const handleComplete = async (bookingId: string) => {
    ...
    try {
      ...
      const error = err as { response?: { data?: { error?: { message?: string } } } };
      toast.error(error?.response?.data?.error?.message || 'Failed to complete');
    } finally {
      setCompleting(null);
    }
  };
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const updated = await bookingApi.cancel(cancelTarget.id, payload);
      setBookings((prev) => prev.map((b) => (b.id === updated.id ? updated : b)));
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      await reviewApi.create(reviewTarget.id, {
        rating: reviewRating,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Bookings"
        description={isOwner ? 'Manage bookings for your businesses.' : 'View and manage your appointments.'}
      />
            ...
            return (
              ...
              >
                <CardContent className="flex items-center justify-between p-4">
                  ...
                  <div className="flex shrink-0 items-center gap-2 pl-4" onClick={(e) => e.stopPropagation()}>
                    {/* Owner actions */}
                    ...
                    {!isOwner && booking.status === 'completed' && !reviewedBookingIds.has(booking.id) && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span>{formatTime(detailBooking.startTime)} – {formatTime(detailBooking.endTime)}</span>
              </div>
              <div className="flex justify-between">
